...
class FMNet(nn.Module):
    def __init__(self, init=True, bNum=3, nblocks=4, input_features=31, num_features=64, out_features=3):
        ...
        self.dics = nn.ModuleList(
            [Dictionary(inChn=num_features, nFeat=num_features, outChn=num_features, layers=2, bNum=bNum) for i in range(self.nblocks)])
        self.reps = nn.ModuleList(
            [Representation(inFeat=num_features, nFeat=num_features, layers=3, outChn=bNum) for i in range(self.nblocks)])

        self.cdic = Dictionary(inChn=num_features * self.nblocks, nFeat=num_features, outChn=num_features, layers=2, bNum=bNum)
